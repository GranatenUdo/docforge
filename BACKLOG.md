...
- **Record demo GIF + embed** (launch) — 30-second screen recording (per the internal branding spec, §8.6). Embed in README + microsite landing.
...
- **Repo cleanup PRs A/B/C — 2026-05-07 (verified done)** — Closed during continuation-plan critical review: PR A items already in (`[project.urls]`, Makefile, .editorconfig, FAQ trimmed; internal-planning-dir move done in PR #16). PR B done (`microsite/scripts/sync-assets.mjs` + `microsite/public/` removed). PR C done (`src/docforge/` package layout, `pyproject.toml` `where = ["src"]`). Plus removed `docforge/docforge/` __pycache__ cruft from before the src/ migration.
- **Publication readiness — 2026-05-07** — Closed: MCP server identifier rename to `docforge`, master CI fork-friendly fix, `deploy/azure/README.md` Phase-4b update (7 resources, embedder section, ACR Standard SKU note), microsite architecture + deployment embedder additions, broken internal-doc link replaced with inline `az postgres restore` command, removal of leftover internal planning artifacts, README HF prereq promotion, v0.3.0 release.
