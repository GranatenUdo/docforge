...
- Documentation polish: `deploy/azure/README.md` updated for v0.3 Phase 4b architecture (7 resources, embedder section, ACR Standard SKU note, updated cost). Microsite architecture and deployment pages note the embedder sidecar split. README's HF token prerequisite promoted from FAQ to quick-start. Broken internal-doc link in the microsite deployment guide replaced with the actual `az postgres flexible-server restore` command. Stale internal planning artifacts removed (a move claimed in 0.2.1 left the originals behind).
...
- Internal planning artifacts moved to a gitignored directory. Historical cross-references inside the moved files are preserved as-is. The single current reference in `microsite/.../deployment.md` was updated.
